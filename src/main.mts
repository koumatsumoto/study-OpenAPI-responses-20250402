...
以下のシステム開発用語を日本語に翻訳し、適切な分類を付与してください。定義の内容を考慮して最も適切な日本語訳を選択し、分類は該当するものをすべて選択してください。
...
{
  "number": "用語番号",
  "name_ja": "定義の内容を考慮した最も適切な日本語訳",
  "categories": ["分類コード1", "分類コード2", ...]
}

**注意事項**
1. 日本語訳は定義の内容を十分に考慮して決定
2. 技術用語として一般的な訳語がない場合は概念を正確に表現する訳を作成
3. 分類は該当するものを過不足なく選択
4. 出力形式から逸脱しない（余分なフィールドを追加しない）
5. 日本語訳はできるだけ簡潔に（長すぎる場合は核心部分のみを抽出）

**翻訳と分類の判断基準**
1. 設計関連の用語 → dev を基本に必要に応じてux/frontend/backendを追加
2. プロセスや手法 → pm または biz を検討
3. 技術要素は具体的な適用領域で分類（frontend/backend/infraなど）
4. セキュリティ関連要素が含まれる場合は必ずsecを追加
